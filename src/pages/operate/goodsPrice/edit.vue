<template>
	<view class="u-page">
		...
		<view class="mt" v-if="!history">
			...
			<view class="ly-flex-pack-justify good">
				<view class="">
					货品：
				</view>
				<Pickers v-model="form.tenantGoodsId" :range='goodsList' placeholder="请选择货品" @change='goodsChange'/>
			</view>
			<view class="main">
...
<script>
	...
	import { addStrategy, getDetailList, historyList, eidtStrategy, goodsList } from '@/config/service/operate/goodsPrice/index.js';
	...
	export default {
		...
		data() {
			return {
				...
				form:{
					id:undefined,
					tenantGoodsId:undefined, // 货品id
					goodsType: undefined,
					goodsBigType: undefined,
					oldId:undefined, // 原生效ID
					name:undefined,
					goodsPoliceDetailBoList:undefined // 完整列表
				}, 
		...
		onLoad(option){
			...
			if(option.id){
				uni.showLoading()
				this.form.tenantGoodsId = +option.tenantGoodsId
		...
		methods: {
			submit(){
				...
				if(this.form.name && this.form.tenantGoodsId){
					if (this.priceData.length === 0) {
					            this.msgSuccess('至少需要一条定价');
					          } else {
					            if (this.effectivePlanNum == 0 || this.priceData.some(res => res.takeEffectStatus === 0)) {
								  this.loading = true
					              this.form.goodsPoliceDetailBoList = this.form.id ? [...this.priceData, ...this.deleteData] : this.priceData
					              this.form.id
					...
					}else if(!this.form.tenantGoodsId){
						this.msgSuccess('请选择货品');
					}
				}
			},
			navigateBack() {
			...
			// 切换货品
			goodsChange(id){
				const goodsInfo = this.originList.find(res => res.id === id)
				if(goodsInfo){
					this.form.goodsBigType = goodsInfo.goodsBigType
